...
def calculate_kl_divergence(p_text, q_text, model, tokenizer):
    ...
    with torch.no_grad():
        ...
        kl_div = torch.sum(p_probs * (torch.log(p_probs + 1e-10) - torch.log(q_probs + 1e-10)), dim=-1)
        return kl_div.item()
...
def pruning_node(state: GraphState, model, tokenizer, threshold=0.35) -> GraphState:
    ...
    for candidate in raw_nodes[1:]:
        info_gain = calculate_kl_divergence(anchor_node["content"], candidate["content"], model, tokenizer)
